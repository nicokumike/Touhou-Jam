...
func play_animation(animation_name: String, loop: bool, track: int = 0, mix_duration: float = -1) -> SpineTrackEntry:
...
func play_animation_asset(animation_asset: SpineAnimationAsset) -> SpineTrackEntry:
	return play_animation(animation_asset.animation_name, animation_asset.loop, animation_asset.track, animation_asset.mix_duration)
